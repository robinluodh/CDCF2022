function cdcf
    ...
    param.net = net;
    param.padding = 1;
    [state, ~] = CDCF_initialize(imread(image), region, param);
    ...
    while true
        ...
        [state, region, confidence] = CDCF_update(state, imread(image));
        % **********************************
...
function [state, location, confidence] = CDCF_update(state, I, varargin)
    ...
    zf = fft2(z);
    [~, vert_delta, horiz_delta, scale_delta, confidence] = Detect(zf, state);
    ...
    for i = 1:length(max_response)
        ...
        if max_response(i) > 0
            ...
            if max_index(i) <= np %åŒ¹é…�
                ...
                %baglocate(:,i)=[d(i,max_index(i),1);d(i,max_index(i),2)]+state.nlabel{max_index(i)-np}-1;
                t = 0.25 * state.bagsz .* ([2 + fix((i - 1) / 5); 2 + mod(i - 1, 5)]) - 0.5 * 125;
                tmp.loc = t;
                tmp.w = 0.01 * (max_response(i) - state.bagresth);
                tmp.xf = bagsf{i};
...
function img_crop = imcrop_multiscale(img, pos, sz, output_sz, yyxx)
    ...
    try
        img_crop = vl_nnbilinearsampler(single(img), g);
    catch e
        e
        img
        error('nnbilinear error');
    end
